...
try:
   ...
   prints(nel(f"{P2}لاتوفر اتصال بالانترنت حاول مره اخرئ",width=80,padding=(0,2),style=f"{color_panel}"));exit()
...
def login_lagi334():
	try:
		...
		print('')
		print('')
		your_cookies = input(f'[{h}={x}] M[للاستمرار حط كوكيز مره اخرئ/ Cookies :{asu} ')
				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							print('')
							print(f"{x}[{h}√{x}]{h} تم اضافه الكوكيز بنجاح ارجع رجوع وسجل دخول مره اخرئ{x} ");time.sleep(1);exit()
...
def menu(my_name,my_id):
	...
	banner()
	print('')
	print(f'\033[2;36mID : '+str(my_id))
	print(f'\033[2;36mIP : {ip}')
	print('')
	print(f"""\033[1;33m01 Crack Public | من الاصدقاء من ايدي واحد 
02 Crack Public [MULTI] | من الاصدقاء من عده ايديات
03 Crack File | من ملف  
""")
	Meledak = input('[•] Pilih  : ')
	print('')
	if Meledak in ['1']:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('> لايمكن ان تستقبل الاداه اكثر من 15 ايدي  اكتب عدد الايديات المطلوبه :'))
		print('')
	except ValueError:
		print(' ')
		exit()
...
def setting():
	...
	print('\033[1;33m1 - RANDOM')
	
	print('')

	hu = input('\033[1;33m[*] Pilih  : ')
	if hu in ['','old']:
...
def getkey():
    ...
    try:
        ...
        print("")
        print('')
        print("\033[0m Telegram: @xglle ");time.sleep(2)
        print ("")
        jalan("\033[0m  @xglle :\033[32m "+crot);time.sleep(1)
        namamu = input('H')
  ...
  #  os.system('clear')
    try:
        ...
        if response.status_code == 200:
            ...
            uascrap.append("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Safari/537.36")
    except requests.exceptions.ConnectionError:
        uascrap.append("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/109.0.0.0 Safari/537.36")
